...
def test_find_slicer_python_linux(monkeypatch):
    ...
    monkeypatch.setattr(subprocess, 'run', lambda cmd, shell, stdout: type('P', (), {'stdout': b'/opt/Slicer/bin/PythonSlicer\n'}))
...
def test_patch_add_and_remove(monkeypatch, tmp_path):
    ...
    assert SlicerCheckWorker.BEGIN_MARKER not in SlicerCheckWorker.read_slicerrc(str(slicerrc))
...
def test_run_detects_slicer_and_modules(monkeypatch, tmp_path):
    ...
    results = []
    w.signal.slicer.connect(lambda cmd, ver, label, state: results.append((cmd, ver, label, state)))

    # monkeypatch find_slicer_python to return a fake path
    monkeypatch.setattr(SlicerCheckWorker, 'find_slicer_python', staticmethod(lambda p: (['/fake/path/bin/PythonSlicer'], '../Slicer')))
