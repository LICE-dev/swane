...
def import_from_path(
    patient: Subject, data_input: DataInputList, dicom_path: str, qtbot
):
    patient.clear_import_folder(data_input)
...
class TestWorkflow:
    ...
    TESTS = {
        ...
        "flair_and_flat1": {
            "data": {DataInputList.FLAIR3D: "singlevol"},
            "preferences": {
                DataInputList.T13D: [
                    ["flat1", "true"],
                ]
        ...
        "asl_ai": {
            "data": {DataInputList.ASL: "singlevol"},
            "preferences": {
                DataInputList.T13D: [
                    ["freesurfer", "true"],
                ],
            ...
            "check_nodes": {
                DataInputList.ASL.value.workflow_name: [["asl_surf_lh"], ["asl_ai"]],
            },
        },
        "asl_base": {
            "data": {DataInputList.ASL: "singlevol"},
        ...
        "dti_oldeddy_tracto": {
            ...
            "check_nodes": {
                ...
                "tract_cst": [["random_seed", "seeds_n", 10]],
            },
        },
        "fmri_base": {
            "data": {
            ...
            "check_nodes": {
                DataInputList["FMRI_0"].value.workflow_name: [
                    [
                        "fmri_0_genSpec",
                        "task_duration",
                        WF_PREFERENCES[DataInputList["FMRI_0"]][
                            "task_duration"
                        ].default,
                    ],
                    [
                    ...
                    [
                        "fmri_0_del_vols",
                        "del_end_vols",
                        WF_PREFERENCES[DataInputList["FMRI_0"]]["del_end_vols"].default,
                    ],
                    ["fmri_0_cluster_2"],
                ],
            },
        ...
        for test_name in TestWorkflow.TESTS:
            ...
            if last_test is not None:
                for data_input in DataInputList:
                    if data_input not in this_test["data"] or (
                        data_input in last_test["data"]
                        and this_test["data"][data_input]
                        != last_test["data"][data_input]
                    ):
            ...
            for data_input in this_test["data"]:
                if (
                    ...
                    and data_input in last_test["data"]
                    and this_test["data"][data_input] == last_test["data"][data_input]
                ):
                    pass
                else:
                    import_from_path(
                        ...
                        data_input,
                        os.path.join(
                            TestWorkflow.DATA_DIR, this_test["data"][data_input]
                        ),
                        qtbot,
                    )

                assert (
                    test_patient.input_state_list[data_input].loaded is True
                ), "%s not loaded for %s" % (data_input, test_name)
            ...
            # Generate workflow
            assert test_patient.input_state_list.is_ref_loaded() is True, (
                "missing t13d for " + test_name
            )
            test_patient.reset_workflow()
            assert (
                test_patient.generate_workflow(generate_graphs=False)
                == SubjectRet.GenWfCompleted
            ), ("Error generating workflow for " + test_name)
            ...
            for workflow_name in this_test["check_nodes"]:
                ...
                if not sub_wf_presence:
                    assert (
                        sub_wf is None
                    ), "There should not be %s subworkflow for %s" % (
                        workflow_name,
                        test_name,
                    )
                else:
                    assert sub_wf is not None, "Cannot find %s subworkflow for %s" % (
                        workflow_name,
                        test_name,
                    )
                    for node_name in this_test["check_nodes"][workflow_name]:
                        ...
                        if not node_presence:
                            assert (
                                node is None
                            ), "There should not be %s in %s subworkflow for %s" % (
                                node_name[0],
                                workflow_name,
                                test_name,
                            )
                            ...
                            assert (
                                node is not None
                            ), "Cannot find %s in %s subworkflow for %s" % (
                                node_name[0],
                                workflow_name,
                                test_name,
                            )
                            if len(node_name) == 3:
                                assert (
                                    hasattr(node.inputs, node_name[1]) is True
                                ), "No %s input in %s not in %s subworkflow for %s" % (
                                    node_name[1],
                                    node_name[0],
                                    workflow_name,
                                    test_name,
                                ...
                                assert input_value == node_name[2], (
                                    "Error for %s input value (%s instead of %s) of %s node in subworkflow %s for %s"
                                    % (
                                        node_name[1],
                                        str(input_value),
                                        str(node_name[2]),
            ...
            if "execute" in this_test and this_test["execute"] is True:
                ...
                qtbot.waitUntil(
                    test_patient.workflow_process.stop_event.is_set, timeout=2000000
                )
                assert (
                    self.last_node_cb == WorkflowSignals.WORKFLOW_STOP
                ), "Workflow finished but not segnaled"
    ...
    def node_callback(self, wf_report: WorkflowReport, test_name: str):
        ...
        if (
            not self.allow_wf_errors
            and wf_report.signal_type == WorkflowSignals.NODE_ERROR
        ):
