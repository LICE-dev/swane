...
class TestCompleteWorkflow:
    TEST_MAIN_WORKING_DIRECTORY = os.path.join(
        TEST_DIR, "complete_workflow", "subjects"
    )
    ...
    def test_complete_workflow(self, qtbot):
        ...
        assert os.path.exists(real_exec_subject_path), (
            "Could not find subj_test in %s" % real_main_working_directory
        )
        os.system("cp " + real_global_config.config_file + " " + os.getcwd())
        test_global_config = ConfigManager(global_base_folder=os.getcwd())
        test_global_config.set_main_working_directory(
            TestCompleteWorkflow.TEST_MAIN_WORKING_DIRECTORY
        )
        os.system(
            "cp -r %s %s"
            % (real_exec_subject_path, TestCompleteWorkflow.TEST_MAIN_WORKING_DIRECTORY)
        )
        text_exec_subject_path = os.path.join(
            TestCompleteWorkflow.TEST_MAIN_WORKING_DIRECTORY, "subj_test"
        )
        assert os.path.exists(
            text_exec_subject_path
        ), "Error in copying subj_test folder"
        ...
        qtbot.waitUntil(
            lambda: subj_tab.isTabEnabled(SubjectTab.EXECTAB), timeout=1000 * 60 * 2
        )
        qtbot.waitUntil(lambda: not subj_tab.is_data_loading(), timeout=1000 * 60 * 2)
        assert subj_tab.isTabEnabled(
            SubjectTab.EXECTAB
        ), "Exectab disabled after data loading"
        ...
        shutil.rmtree(
            os.path.join(text_exec_subject_path, LOG_DIR_NAME), ignore_errors=True
        )

        # Generate workflow
        assert (
            subj_tab.generate_workflow_button.isEnabled()
        ), "Generate workflowbutton is disabled"
        ...
        # Clear previous wf output copied from real folder
        shutil.rmtree(
            os.path.join(text_exec_subject_path, test_subject.workflow.name),
            ignore_errors=True,
        )
        ...
        qtbot.waitUntil(
            lambda: test_subject.workflow_monitor_work is not None, timeout=1000 * 5
        )
        self.wf_error = False
        test_subject.workflow_monitor_work.signal.log_msg.connect(
            self.update_node_callback
        )
        qtbot.waitUntil(
            lambda: not test_subject.is_workflow_process_alive(), timeout=1000 * 60 * 5
        )
        assert self.wf_error is False, "Node error during workflow"
        assert subj_tab.isTabEnabled(
            SubjectTab.RESULTTAB
        ), "No result after workflow execution"

        # Generate Slicer Scene
        subj_tab.setCurrentIndex(SubjectTab.RESULTTAB)
        assert (
            subj_tab.generate_scene_button.isEnabled()
        ), "Generate scene button not enabled"
        assert (
            subj_tab.open_results_directory_button.isEnabled()
        ), "Open result button not enabled"
