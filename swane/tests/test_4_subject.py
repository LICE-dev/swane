...
class TestSubject:
    ...
    def test_subject(self, global_config, dependency_manager, qtbot):
        assert (
            ...
            == TestSubject.TEST_MAIN_WORKING_DIRECTORY
        ), "Bad global config fixture"

        # Testing creation of new subject
        test_subject = Subject(global_config, dependency_manager)
        assert (
            test_subject.create_new_subject_dir("Invalid with space")
            == SubjectRet.PathBlankSpaces
        ), "Error with new subject with blank space"
        assert (
            test_subject.create_new_subject_dir("Invalid*char")
            == SubjectRet.PathBlankSpaces
        ), "Error with new subject with special character"
        assert (
            test_subject.create_new_subject_dir(None) == SubjectRet.FolderNotFound
        ), "Error with new subject with None name"
        assert (
            test_subject.create_new_subject_dir("") == SubjectRet.FolderNotFound
        ), "Error with new subject with empty name"
        assert (
            test_subject.create_new_subject_dir(TestSubject.TEST_SUBJECT_NAME)
            == SubjectRet.ValidFolder
        ), "Error with new subject with valid name"
        assert (
            test_subject.create_new_subject_dir(TestSubject.TEST_SUBJECT_NAME)
            == SubjectRet.FolderAlreadyExists
        ), "Error with new subject with already existing name"
        test_subject_folder = test_subject.folder

        # Testing function check_pt_folder
        test_subject = Subject(global_config, dependency_manager)
        path_with_spaces = os.path.join(
            TestSubject.TEST_MAIN_WORKING_DIRECTORY, "sub space"
        )
        assert (
            test_subject.check_subject_folder(path_with_spaces)
        ...
        assert (
            test_subject.check_subject_folder(path_with_spaces)
            == SubjectRet.PathBlankSpaces
        ), "Error with path containing blank spaces"
        assert (
            test_subject.check_subject_folder(os.path.expanduser("~"))
            == SubjectRet.FolderOutsideMain
        ), "Error with folder outside main working directory"
        path_without_subtree = os.path.join(
            TestSubject.TEST_MAIN_WORKING_DIRECTORY, "pt_without_subtree"
        )
        os.makedirs(path_without_subtree)
        assert (
            test_subject.check_subject_folder(path_without_subtree)
            == SubjectRet.InvalidFolderTree
        ), "Error with folder without subtree"
        assert (
            test_subject.check_subject_folder(test_subject_folder)
            == SubjectRet.ValidFolder
        ), "Error loading a subject valid folder"
        ...
        assert (
            test_subject.check_subject_folder(path_without_subtree)
            == SubjectRet.ValidFolder
        ), "Error fixing subject subtree"

        # Load subject and import
        assert (
            test_subject.load(test_subject_folder) == SubjectRet.ValidFolder
        ), "Error loading empty subject"
        ...
        assert (
            import_ret == SubjectRet.DataImportCompleted
        ), "Importing valid series error"
        ...
        assert (
            import_ret == SubjectRet.DataInputNonEmpty
        ...
        assert (
            import_ret == SubjectRet.DataImportErrorVolumesMin
        ), "Min volumes check error"
        ...
        assert (
            import_ret == SubjectRet.DataImportErrorModality
        ), "Incorrect modality check error"
        ...
        assert (
            import_ret == SubjectRet.DataImportCompleted
        ), "Incorrect modality force error"
        ...
        assert (
            import_ret == SubjectRet.DataImportErrorVolumesMax
        ), "Max volumes check error error"

        # clear folder
        clear_ret = test_subject.clear_import_folder(DataInputList.T13D)
        assert clear_ret is True, "Clear folder error"
        assert (
            test_subject.dicom_folder_count(DataInputList.T13D) == 0
        ), "Folder not empty after clear error"
