...
class DipyCsdFit(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # Load float32 rather than get_fdata's float64 default: dipy's CSD
        # upcasts to float64 internally, so shm_coeff is unchanged (~1e-8) while
        # the input buffer's RAM is halved (user decision, 2026-09-07 -- all
        # dipy nodes load float32).
