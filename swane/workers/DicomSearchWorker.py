...
class DicomSearchWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            for dicom_loc in self.unsorted_list:
                ...
                self.dicom_tree[subject_id][study_instance_uid][series_number].append(
                    dicom_loc
                )
    ...
    def get_series_list(
        self, subject: str, exam: pydicom.uid.UID
    ...
    def get_series_nvol(
        self, subject: str, exam: pydicom.uid.UID, series: pydicom.valuerep.IS
    ) -> int:
        """
