...
def main(argv=None):
    """Headless endrocranium segmentation entrypoint callable from tests.

    Accepts an argv-like list (argv[0] is script name). When run under Slicer the
    CLI behaviour is preserved by calling main(sys.argv).
    """
    if argv is None:
        argv = sys.argv[1:]
